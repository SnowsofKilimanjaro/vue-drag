export default class Line {
  constructor (ctx, id, type, start, end, color, edgeColor) {
    this.ctx = ctx
    this.type = type
    this.start = start
    this.end = end
  ...
  draw () {
    ...
    let alpha = Math.atan2(height, width)// 计算任意两点的连线的倾角
    ...
    this.ctx.translate(this.start.x, this.start.y)// 将画布原点平移到（x,y）
